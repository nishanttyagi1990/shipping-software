...
const validate = {
  phone: s => (s.length < 10 && s.length >10 ? "" : "Phone length is not correct"),
};
      ...
      columns: [
        ...
        <FormControlLabel style={popUpStyle.checkboxPosition}
          control={<Checkbox 
            ...
            onChange={()=>{handleChangeCheckbox(rowData.customerId)}}
        ...
        onClick={()=>{handleChangeCheckbox(rowData.customerId)}}
          ...
          control={<Checkbox 
            ...
            checked={
                (() => {
                  ...
                  if (rowData.customerId  === parseInt(ids9[i])){
                    return (
                        true
                      )
                  }
...
Select State</MenuItem>
   ...
   // fetchShiphypeCompleteStep();
    fetchCustomerList();
     ...
     const fetchCustomerList = ()=>{
      ...
      setLoading(true);
      shiphypeservice.fetchCustomerList(uuid)
      .then(response => {
  ...
  aarpi.push(customerId);
      shiphypeservice.deleteCustomer(aarpi)
      .then(response => {
       console.log("status",response.status);
            if(response.status === true) {
              setLoading(false);
    ...
    const deleteCustomer1 = ()=>{
      setLoading(true);
      shiphypeservice.deleteCustomer(changedWarehouseid)
      .then(response => {
       console.log("status",response.status);
            if(response.status === true) {
              setLoading(false);
    ...
    const fetchProductListById = (User)=>{
      ...
      //const userid=5;
      setLoading(true);
      shiphypeservice.fetchCustomerList(User)
      .then(response => {
       console.log("status",response.status);
            if(response.status === true) {
              setLoading(false);
