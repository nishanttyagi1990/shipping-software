...
export default function SelectProduct(props) {
  ...
  const onNextfunction = () => {
    ...
    props.handleNextPage("SelectOrderMainScreen");
    // props.handleNextPage(8);
  };
  ...
  const handleChangeCheckbox = (data, shippedquantity, rowData,isrow) => {
    ...
    if (rowData.torontostock !== null && rowData.losangelesstock !== null) {
      if (
        0 >=
        parseInt(rowData.losangelesstock) + parseInt(rowData.torontostock)
      ) {
    ...
    if (flag90 === true) {
      ...
      if (shipmentId !== 0) {
        ...
        if (event.target.checked == true || isrow === true) {
          ...
          AsyncStorage.setItem("ProductSelect1", JSON.stringify(ProductSelect));
        } else {
          ProductSelect.map((item, index) => {
            if (ProductSelect[index].customproductId == data) {
              ProductSelect.splice(index, 1);
        ...
        if (response.status === true) {
          ...
          if (response.data.length !== 0) {
            ...
            if (response.data[0].productid.length !== 0) {
              ...
              for (var k = 0; k < response.data[0].productid.length; k++) {
                ShippedquantityNew.push({
                  shippedquantity:
                    response.data[0].productid[k].productquantity,
                });
              }
