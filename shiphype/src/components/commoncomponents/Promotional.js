...
var uuid=0;
 const fetchCustomePackageingList = ()=>{
  ...
  var packagin=[];
    setLoading(true);
    shiphypeservice.fetchCustomePaching(uuid,2)
    .then(response => {
  ...
  const addNewPackaging = (itemsku,itemname,tor,los)=>{
    ...
    shiphypeservice.addCustomePacking1(itemsku,itemname,2,tor,los,uuid)
    .then(response => {
     console.log("status",response.status);
          if(response.status === true) {
            setLoading(false);
  ...
  const updataExistsPackaging = (packagging_id,assignsku,packaggingname,tor,los,useridid)=>{
    ...
    shiphypeservice.updatePackageing1(packagging_id,assignsku,packaggingname,2,tor,los,useridid)
    .then(response => {
     console.log("status",response.status);
          if(response.status === true) {
            setLoading(false);
...
const fetchProductListById = (User)=>{
  ...
  //const userid=5;
  setLoading(true);
  shiphypeservice.fetchCustomePaching(User,2)
  .then(response => {
   console.log("status",response.status);
        if(response.status === true) {
          setLoading(false);
...
<View >
     ...
     />
        ...
        title={<Text style={{ fontSize: '13px',
            ...
            transition : 'all 0.25s',}}>Promotional Inserts List</Text> }
