...
export default function Slide17(props) {
      ...
      .then(response => {
        ...
        if (response.status === true) {
          ...
          if (shipmentId !== 0) {
            ...
            AsyncStorage.multiGet(['CustomPackges']).then(data => {
              if (data[0][1] != null) {
                CustomPackges = JSON.parse(data[0][1]);

                if (CustomPackges.length > 0) {
                  CustomPackges.forEach(item => {
            ...
            if (response.data[0].getshippingpackaging.length !== 0) {
              ...
              if (response.data[0].getshippingtypepackaging.length !== 0) {
                for (
                  ...
                  ids3.push(
                    response.data[0].getshippingtypepackaging[
                      i
                    ].packagingtypeId,
                  );
                }
              ...
              NewQTYupdate = response.data[0].getshippingpackaging;
              ...
              setStateproduct(prevState => {
                ...
                data.map((item, index) => {
                  NewQTYupdate.map((item1, index1) => {
                    if (item1.packagingId == item.packaggingId) {
                      console.log('in===');
                      item.packagingquantity = item1.packagingquantity;
                    }
                  });

                  Data.map((item2, index2) => {
                    if (item2.packaggingId == item.packaggingId) {
                      NewSelectedCustomerow.push(item);
                    }
                  });
                });
                AsyncStorage.setItem(
                  'NewSelectedCustomerowData',
                  JSON.stringify(NewSelectedCustomerow),
                );
                return {...prevState, data};
              });
  ...
  const onNextfunction = () => {
    ...
    props.handleNextPage('orderSelectPromotional');
  };
  ...
  const handleChangeCheckbox = (data, ID) => {
    if (shipmentId !== 0) {
      if (event.target.checked == true) {
        NewSelectedCustomerow.push(data);
        console.log(NewSelectedCustomerow);
        ...
        NewSelectedCustomerow.map((item, index) => {
          console.log(NewSelectedCustomerow[index].packagingId == ID);

          if (NewSelectedCustomerow[index].packaggingId == ID) {
            console.log('true');
            NewSelectedCustomerow.splice(index, 1);
            NewSelectedCustomerow = NewSelectedCustomerow;
            console.log(NewSelectedCustomerow);
      ...
      if (event.target.checked == true) {
        CustomPackges.push(data);

        AsyncStorage.setItem('CustomPackges', JSON.stringify(CustomPackges));
      } else {
        CustomPackges.map((item, index) => {
          if (CustomPackges[index].packaggingId == ID) {
            CustomPackges.splice(index, 1);
            CustomPackges = CustomPackges;
          }
        });
        AsyncStorage.setItem('CustomPackges', JSON.stringify(CustomPackges));
      }
