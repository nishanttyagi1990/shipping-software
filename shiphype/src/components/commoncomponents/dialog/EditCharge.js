...
export default function DeleteCard(props) {
  ...
  const { openeditcharge } = props;
  const userid=props.userid;
  const data=props.chargeData;
  ...
  useEffect(() => {
    ...
    //     ...formState,
    //     values: {
    //       ...formState.values,Amount: data.totalamount,
    //       checkFrom : false
    //     },
    //     touched:{
    //       ...formState.touched,
    ...
    //    }));
       ...
       setFormState(formState => ({
        ...formState,
        values: {
          ...formState.values,Amount: Math.abs(data.transact),
          checkFrom : false
        },
        touched:{
          ...formState.touched,
       ...
       setFormState(formState => ({
        ...formState,
        values: {
          ...formState.values,Description: data.description,
          checkFrom : false
        },
        touched:{
          ...formState.touched,
  ...
  const paymentDeduct=()=>{
    ...
    const DeductAmount = formState.values.Amount;
    //const currency=selectcurrency.currency;
    setLoading(true);
      shiphypeservice
        .paymentDeductUpdate(data.currenttransactionId,userid,DeductAmount,description)
        .then((response) => {
            ...
            <Grid container spacing={1} justify="center">
              ...
              {/* <Grid item xs={10} justify="center">
                <TextField
                  ...
                  variant="outlined"
                  fullWidth
                  error={hasError("DeductAmount")}
                  helperText={
                    hasError("DeductAmount") ? formState.errors.DeductAmount[0] : null
                  }
                  placeholder="Enter Deduct Amount"
                  size="small"
                  type="text"
                  onChange={handleChange("DeductAmount")}
                  className={classes.profileMargin1}
                  value={formState.values.DeductAmount || ""}
                />
              </Grid> */}
              {/* <Grid
                item
