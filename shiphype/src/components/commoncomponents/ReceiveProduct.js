...
export default function Slide17(props) {
   ...
   const [state, setState] = React.useState({
    ...
    },
    {
      ...
      {
        title: 'Recieve Quantity',
        field: 'receivequantity',
        type: 'text',
        // render: rowData => <Text>
        // {rowData.relatedshippingproductdetails[0].receivequantity} </Text>
      },
      { title: 'Product', field: 'productname',type: 'text',editable: 'never'
      
    },
     
      { title: 'Bin Assigned', field: 'binassigned', type: 'text',
      // render: rowData => <Text>
...
const isDelete='true';
    ...
    const deleteProduct = {
        ...
        productID: packagging_id,
        is_deleted: true,
    }

    shiphypeservice.deleteProductFromShipemnt(deleteProduct)
    .then(response => {
     console.log("status",response.status);
          if(response.status === true) {
            setOpen(true);
            setType('success');
            setMsg('Successfully Delete Product From This Shipment');
  ...
  const deletePromotionalPacking = (packagging_id)=>{
    const isDelete='true';
    
    
    var promodata=[];
    promodata.push(packagging_id);

const deletePromo = {
  shippingID: shipmentId,
  userid: userid,
  promotionalinsertids:packagging_id,
  is_deleted: true,
}
        ...
        shiphypeservice.updatePromotionalShipment(deletePromo)
        .then(response => {
         console.log("status",response.status);
              if(response.status === true) {
                setOpen(true);
                setType('success');
                setMsg('Successfully Delete Prmotional Inserts From This Shipment');
                setStatus(response.status);
                setLoading(false);
      ...
      const deleteCustomePackingData = (packagging_id)=>{
        const isDelete='true';
        
         
var promo=[];
promo.push(packagging_id);
const deleteCusotm = {
  shippingID: shipmentId,
  userid: userid,
  packagingids:packagging_id,
  is_deleted: true,
            ...
            shiphypeservice.updateCustomShipment(deleteCusotm)
            .then(response => {
             console.log("status",response.status);
                  if(response.status === true) {
                    setOpen(true);
                    setType('success');
                    setMsg('Successfully Delete Custom Packaging From This Shipment');
                    setStatus(response.status);
                    setLoading(false);
  ...
  const updataExistsPackaging = (packagging_id,assignsku,packaggingname,shippedquantity)=>{
...
const updateProduct = {
  ...
  binassigned:packaggingname,
  receivequantity:assignsku,
  is_deleted: false,
}

    shiphypeservice.updateProductShipment(updateProduct)
    .then(response => {
     console.log("status",response.status);
          if(response.status === true) {
          setOpen(true);
          setType('success');
          setMsg('Successfully Update Deatils');
            setStatus(response.status);
            setLoading(false);
  ...
  const updataPromotionalPackaging = (packagging_id,assignsku,assignsku1,bin)=>{
    const isDelete='false';
    // const [customDataUpdate, setcusotmDataUpdate] = useState([{ shippingID: shipmentId ,userid :userid,packagingids:[],
    //   packagingquantity:[]}]);
...
promo.push(packagging_id);
var promoq=[];
promoq.push(assignsku);
const updatePromo = {
  shippingID: shipmentId,
  userid: userid,
  promotionalinsertids:packagging_id,
  promotionalinsertsquantity:assignsku,
  binassigned:bin,
  receivequantity:assignsku1,
  is_deleted: false,
  
}

    shiphypeservice.updatePromotionalShipment(updatePromo)
    .then(response => {
     console.log("status",response.status);
          if(response.status === true) {
          setOpen(true);
  ...
  const updataCsutomPackaging = (packagging_id,assignsku,assignsku1,bin)=>{
    const isDelete='false';
...
const updateCusotm = {
  ...
  is_deleted: false,
    ...
    shiphypeservice.updateCustomShipment(updateCusotm)
    .then(response => {
     console.log("status",response.status);
          if(response.status === true) {
          setOpen(true);
  ...
  />
    ...
    return (  
        ...
        <View className={classes.appBarSpacer} />
          ...
          <Text style={popUpStyle.breadCrundCss2}> RECIEVE INVENTORY / SHIPMENT PRODUCTS {'\n'} </Text> 
    ...
    onClick={()=>{props.handleNextPage('10')}}
    >
    Back
  </ColorButton>
              </Grid>
      ...
      />
        ...
        title={<Text style={{ fontSize: '13px',
           // fontWeight: '700',
            ...
            transition : 'all 0.25s',}}>Recieve Inventory Products - {shipmentId}</Text> }
            ...
            setTimeout(() => {
              {
                ...
                const packaggingId=(dataproduct[index].productId);
                updataExistsPackaging(packaggingId,newData.receivequantity,newData.binassigned, newData.shippedquantity);
              }
              resolve()
            }, 1000)
          }),
...
<MaterialTable
        title={<Text style={{ fontSize: '13px',
           // fontWeight: '700',
            ...
            transition : 'all 0.25s',}}>Recieve Inventory Promotional - {shipmentId}</Text> }
      ...
      editable={{
        ...
        onRowUpdate: (newData, oldData) =>
          new Promise((resolve, reject) => {
            setTimeout(() => {
              {
                const data = dataPromotional;
                const index = data.indexOf(oldData);
                const packaggingId=(dataPromotional[index].promotionalinserts);
                updataPromotionalPackaging(packaggingId,newData.promotionalinsertquantity,newData.receivequantity,newData.binassigned,);
              }
              resolve()
            }, 1000)
          }),
            ...
            setTimeout(() => {
              {
                ...
                const index = data.indexOf(oldData);
                const packaggingId=(dataPromotional[index].promotionalinserts);
                deletePromotionalPacking(packaggingId);
              }
              resolve()
            }, 1000)
          }),
...
<MaterialTable
        title={<Text style={{ fontSize: '13px',
           // fontWeight: '700',
            ...
            transition : 'all 0.25s',}}>Recieve Inventory Custom Packaging - {shipmentId}</Text> }
            ...
            setTimeout(() => {
              {
                ...
                const index = data.indexOf(oldData);
                const packaggingId=(dataCustom[index].packagingId);
                updataCsutomPackaging(packaggingId,newData.packagingquantity,newData.receivequantity,newData.binassigned,);
              }
              resolve()
            }, 1000)
          }),
            ...
            setTimeout(() => {
              {
                ...
                const index = data.indexOf(oldData);
                const packaggingId=(dataCustom[index].packagingId);
                deleteCustomePackingData(packaggingId);
              }
              resolve()
            }, 1000)
          }),
