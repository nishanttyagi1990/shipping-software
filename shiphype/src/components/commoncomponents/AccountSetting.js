...
const fetchUserDetails1 = (user)=>{
    ...
    .then(response => {
     console.log("status",response.status);
         ...
         bindData(response.data.userdetails);
...
<View>
          ...
          <Text style={popUpStyle.breadCrundCss2}> ACCOUNT SETTING {'\n'} </Text> 
          ...
          fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',}}>
            ...
            {(() => {
              if (userstatus === true){
                  return (
                  <Text >
                     {userdetails.useraddressline}  {'\n'}
                     {userdetails.usercity}  {'\n'}
                     {userdetails.userstateprovinceCode}  {'\n'}
