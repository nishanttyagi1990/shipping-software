...
const handleGetShipmentId=(rowdata)=>{
  props.getShipmentIdFromOrder(rowdata);
   ...
   const [state, setState] = React.useState({
    ...
    columns: [
      ...
      { title: 'Order ID',
      field: 'internalorderId',type: 'text',
  ...
  { title: 'Date Created', field: 'orderdate',type: 'date'},
  {
    ...
    render: rowData => <Text>
          ...
          <ColorButtonProcessed> Processing </ColorButtonProcessed>
                 ...
                 {
                   return(
                    <ColorButtonShipped> Shipped </ColorButtonShipped>
                   )
                 }
                 ...
                 else if(rowData.orderstatus===8)
                 {
                  ...
                  return(
                    <ColorButtonNew> New Order </ColorButtonNew>
                                   )
                 }
  ...
  render: rowData =>
      ...
      transition : 'all 0.25s',}}>{rowData.customer.companyname}</Text>
  ...
  render: rowData =>
      <Text style={{ fontSize: '11px', 
      fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',
      transition : 'all 0.25s',}}>{rowData.customer.addressline1}</Text>
        },
  { title: 'City', field: 'city',type: 'text',
  render: rowData =>
      <Text style={{ fontSize: '11px', 
      fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',
      transition : 'all 0.25s',}}>{rowData.customer.city}</Text>
        },
  { title: 'State/Province', field: 'state',type: 'text',
  render: rowData =>
      <Text style={{ fontSize: '11px', 
      fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',
      transition : 'all 0.25s',}}>{rowData.customer.state}</Text>
        },
  { title: 'Zip/Postal Code', field: 'zip',type: 'text',
  render: rowData =>
  <Text style={{ fontSize: '11px', 
  fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',
  transition : 'all 0.25s',}}>{rowData.customer.zip}</Text>},
  { title: 'Country', field: 'country',type: 'text',
  render: rowData =>
  <Text style={{ fontSize: '11px', 
  fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',
  transition : 'all 0.25s',}}>{rowData.customer.country}</Text>},
  { title: 'Telephone', field: 'phone',type: 'text',
  render: rowData =>
  <Text style={{ fontSize: '11px', 
  fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',
  transition : 'all 0.25s',}}>{rowData.customer.phone}</Text>},
  { title: 'Email', field: 'email',type: 'text',
  render: rowData =>
  <Text style={{ fontSize: '11px', 
  fontFamily: '-apple-system, BlinkMacSystemFont, "Inter UI", Roboto, sans-serif',
  transition : 'all 0.25s',}}>{rowData.customer.email}</Text>},
  { title: 'Additional Notes', field: 'recipientname9',type: 'text'},
  
  { title: 'Date Shipped', field: 'shipdate',type: 'date'},
  
  { title: 'Carrier', field: 'shippingpolicy', type: 'text'},
 ...
 const fetchOrderDetails = ()=>{
    ...
    //const userid=5;
    setLoading(true);
    shiphypeservice.fetchOrderDetails(internalorder_id,userid)
    .then(response => {
...
const fetchPackingList = (userid) => {
  ...
  return (
   ...
   <View className={classes.appBarSpacer} />
     ...
     <Text style={popUpStyle.breadCrundCss2}> SELECTED ORDER DETAIL {'\n'} </Text> 
    ...
    <MaterialTable
      ...
      //  title="."
        ...
        components={{
          ...
          Toolbar: props => (
            <View style={{ backgroundColor: '#fff',height:'45px',fontSize:'12px',padding:0 }}>
              <MTableToolbar {...props} 
             style={{
              paddingBottom:'0px',
              paddingTop:'0px'
     ...
     <Grid container justify="space-between" spacing={2} style={{marginTop:'10px'}}>
     ...
     <MaterialTable
      ...
      transition : 'all 0.25s',}}>Order Product</Text> }
