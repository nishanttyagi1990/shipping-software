  ...
  var ids3=[];
   ...
   const [state, setState] = React.useState({
    ...
    columns: [
      ...
      field: 'packaggingId',type: 'text',editable: 'never',hidden:true,
...
for(let i=0;i<response.data[0].getshippingpromotionalinsert.length;i++){
ids.push(response.data[0].getshippingpromotionalinsert[i].promotionalinserts);
}

  
  if(response.data[0].getshippingtypepackaging.length !== 0){
    for(let i=0;i<response.data[0].getshippingtypepackaging.length;i++){
      ids3.push(response.data[0].getshippingtypepackaging[i].packagingtypeId);
    }
...
<View >
            ...
            </Link><Text style={popUpStyle.breadCrundCss}> TRANSFER INVENTORY /</Text>
...
Save
        ...
        title={<Text style={{ fontSize: '13px',
           fontWeight: '700',
            ...
            transition : 'all 0.25s',}}>Select Promotional Inserts</Text> }
