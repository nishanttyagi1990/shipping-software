 ...
 * @param {*} props
 */
export default function SquareSpaceIntegrationUI(props) {
  const classes = useStyles();

  const { opensquarespace } = props;
  ...
  const sendShopfyDetails = () => {
    ...
    shiphypeservice.checkShopifyIntegration(userid,6,appStorename)
          .then(response => {
           console.log("status",response.status);
                if(response.status === true) {
          ...
          // const url1="https://";
          // const url2=appname;
          // const url3=".myshopify.com/admin/oauth/authorize?client_id=4e29618b7d91656eed161104c6f5238a&redirect_uri=https://app.shiphype.com/&response_type=code&scope=write_orders,write_products,write_customers,read_orders,read_products,read_customers&state=24abdb4a773b68d59d0e6b95355b4eceb2d9af80e12209fb";
          // const url4=url1+url2+url3;
          var url="https://login.squarespace.com/api/1/login/oauth/provider/authorize?client_id=gpr94X6Bo74yWuQpjIj2WnbfA0IyfwsY&redirect_uri=https://app.shiphype.com/&scope=website.orders,website.products&state=nk98686&access_type=offline";
          window.open(url, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=150,width="+{screenWidth}+",height="+{screenHeight}+"fullscreen=yes");
         // HandleClose2();
         flag=true;
         fetchUserIntegrationShopify(userid);
...
SquareSpaceIntegrationUI.propTypes = {
  opensquarespace: PropTypes.bool,
