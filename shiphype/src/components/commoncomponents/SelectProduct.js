  ...
  const [state, setState] = React.useState({
    columns: [
      ...
      },
      {
        ...
        lookup: packingdata,hidden:true,
        // render: (rowData) => (
        //   <FormControlLabel
        //     onClick={() => {
        //       handleChangeCheckbox(
      ...
      .then((response) => {
        ...
        if (response.status === true) {
          ...
          if (shipmentId !== 0) {
            ...
            AsyncStorage.multiGet(["ProductSelect"]).then((data) => {
              if (data[0][1] != null) {
          ...
          if (response.data.length !== 0) {
            ...
            if (response.data[0].productid.length !== 0) {
              for (let i = 0; i < response.data[0].productid.length; i++) {
                ids.push(response.data[0].productid[i]);
              }
              ...
              for (
                var k = 0;
                k < response.data[0].shippedquantity.length;
                k++
              ) {
                ShippedquantityNew.push({
                  shippedquantity: response.data[0].shippedquantity[k],
                });
              }
      ...
      <View className={classes.paper9}>
        <View style={popUpStyle.paddingSide1}>
          ...
          <MaterialTable
            ...
            //             stateproduct.data[index].customproductId;
            //           console.log("sku", newData.productsku);
            //           console.log("name", newData.productname);
